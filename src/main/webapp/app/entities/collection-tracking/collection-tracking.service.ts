...
export class CollectionTrackingService {
  ...
  protected convertDateFromClient(collectionTracking: ICollectionTracking): ICollectionTracking {
    const copy: ICollectionTracking = Object.assign({}, collectionTracking, {
      pptDate:
        collectionTracking.pptDate && collectionTracking.pptDate.isValid() ? collectionTracking.pptDate.format(DATE_FORMAT) : undefined,
    });
    return copy;
  }
  ...
  protected convertDateFromServer(res: EntityResponseType): EntityResponseType {
    if (res.body) {
      res.body.pptDate = res.body.pptDate ? moment(res.body.pptDate) : undefined;
    }
    return res;
  }
  ...
  protected convertDateArrayFromServer(res: EntityArrayResponseType): EntityArrayResponseType {
    if (res.body) {
      res.body.forEach((collectionTracking: ICollectionTracking) => {
        collectionTracking.pptDate = collectionTracking.pptDate ? moment(collectionTracking.pptDate) : undefined;
      });
    }
    return res;
  }
